...
## Reviewer 1, Comment 9 (Terrain Switching Sequence — Variable Topography Physical Validation)

**Reviewer Comment:** The behavior of the robot when moving from a smooth ramp to an unconstrained, rough physical surface is not clear. The authors should illustrate the step-by-step reconfiguration sequence triggered by the controller to evaluate the transition stability.

**Change made:** *Location: Subsubsection 3.2.4 (Response to Variable Topographies).*
> "Frame 5 (Figure~\ref{fig:real_terrain_frame5}): Upon transitioning from the ramp terminus to the Perlin-noise rough terrain, the wheels encounter a surface characterised by high-variance height fluctuations and non-uniform contact geometry. Under these conditions, the rolling effectors experience critical slippage and a severe loss of traction, resulting in a measurable stagnation state: the platform's forward progress index drops sharply, and the MLP outputs trigger a sustained high-activity signal in the stagnation detection channel. This adverse traction event is precisely the destabilising input that the basal-ganglia network is designed to detect and resolve. Frame 6 (Figure~\ref{fig:real_terrain_frame6}): Upon detecting the prolonged stagnation condition and the associated collapse of the locomotion progress metrics, the basal-ganglia-inspired arbitration architecture executes a reactive state transition, disinhibiting the quadrupedal walking channel and suppressing the differential rolling channel. The limbs deploy from their retracted configuration, re-establishing the full four-point articulated contact posture. This reconfiguration restores the support polygon, eliminates the wheel-slip condition, and enables the platform to traverse the rough Perlin-noise terrain successfully."

**Justification:** We have fully resolved this comment by integrating a detailed, 6-frame sequential behavioral analysis into Subsubsection 3.2.4, backed by an experimental pipeline of 15 continuous trials. Figures \ref{fig:real_terrain_frame1} through \ref{fig:real_terrain_frame6} clearly map the chronological adaptation of the platform as it climbs the double ramp in quadrupedal mode, shifts to differential rolling on the elevated plane, and reactively switches back to quadrupedal walking once the wheels encounter the high-variance slip conditions of the Perlin-noise rough surface. This modification provides the qualitative transparency requested by the reviewer, with all additions highlighted in blue to ease the verification pass.
